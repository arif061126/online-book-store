...
export class BookService {
  ...
  /*searchBookByKeyword(keyword:string):Observable<Book[]>{
    return this.httpClient.get<GetResponseBooks>(`${this.baseURL}/search/searchByKeyword?keyword=${keyword}`).pipe(
      map(response=>response._embedded.books)
    );
  }*/

  searchBookByKeyword(keyword:string, currentPage:number, pageSize:number):Observable<GetResponseBooks>{
    return this.httpClient
      .get<GetResponseBooks>(`${this.baseURL}/search/searchByKeyword?keyword=${keyword}&page=${currentPage}&size=${pageSize}`);
  }
  ...
  getAllBooks(pageSize:number):Observable<GetResponseBooks>{
    return this.httpClient.get<GetResponseBooks>(`${this.baseURL}/size=${pageSize}`);
  }
}
